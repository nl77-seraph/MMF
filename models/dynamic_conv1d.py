...
def test_dynamic_conv1d():
    ...
    # 创建模拟数据
    query_features = torch.randn(batch_size, seq_len, feature_dim)
    support_weights = torch.randn(num_classes, feature_dim)
    
    print(f"\n输入形状:")
    print(f"  查询特征: {query_features.shape}")
    print(f"  支持权重: {support_weights.shape}")
    
    # 创建重加权模块
    reweighting_module = FeatureReweightingModule(feature_dim, kernel_size)
    
    # 前向传播
    with torch.no_grad():
        reweighted_features = reweighting_module(query_features, support_weights)
    
    print(f"\n输出形状:")
    print(f"  重加权特征: {reweighted_features.shape}")
    
    # 验证形状
    expected_shape = reweighting_module.get_output_shape(batch_size, seq_len, num_classes)
    print(f"  期望形状: {expected_shape}")
    
    assert reweighted_features.shape == expected_shape, \
        f"输出形状不匹配: {reweighted_features.shape} != {expected_shape}"
